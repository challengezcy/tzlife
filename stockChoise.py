...
def calculate_stock_fluctuation_HZ(stock):
    ...
    for j in (1,):  # (1,2,3,5)
        ...
        for i in range(DAYNUM, -1, -1):
            if data.close[i + j] < data.high[i] and round(
                    (data.high[i] - data.close[i + j]) / (data.close[i + j]), 2) > wave:
                fluc_up = fluc_up + 1
            if data.close[i + j] > data.low[i] and round(
                    (data.close[i + j] - data.low[i]) / (data.close[i + j]), 2) > wave:
                fluc_dw = fluc_dw + 1
        ...
        if (stock_item['aT'] < EXPECT or stock_item['aT']
                > NOT_GOOD or stock_item['d'] > NO_DIFF):
            return 0
        stock_item['score'] = 0
