...
#买入股票数是100的整数倍
Trade_count = round((((float)(Trade_mount))/((float)(base_price)))/(GRID_COUNT*100), 0) * 100
...
def generate_base_grid():
	for i in range(1, GRID_COUNT + 1):
		if i <= UP_BASE_PRICE_GRID_COUNT:
			...
			ws.cell(row=i, column=3).value = trade_count - round((UP_BASE_PRICE_GRID_COUNT-i)/2, 0) * 100
			ws.cell(row=i, column=4).value = ws.cell(row=i, column=3).value * ws.cell(row=i, column=1).value
		else:
			ref_price = ws.cell(row=i-1, column=1).value
			ws.cell(row=i, column=1).value = ((float)(ref_price))*((1000-(int)(10*down_range+(15*(i-UP_BASE_PRICE_GRID_COUNT))))/1000)
			ws.cell(row=i, column=2).value = ws.cell(row=i, column=1).value * ((10000+(int)(up_range*100 + (i-UP_BASE_PRICE_GRID_COUNT)*40))/10000)
			ws.cell(row=i, column=1).value = round(ws.cell(row=i, column=1).value, 2)
			ws.cell(row=i, column=2).value = round(ws.cell(row=i, column=2).value, 2)

			#对低价股调整有意义，根据实际情况看是否需要调整
			ws.cell(row=i, column=3).value = trade_count + (i - UP_BASE_PRICE_GRID_COUNT) * 100
			ws.cell(row=i, column=4).value = ws.cell(row=i, column=3).value * ws.cell(row=i, column=1).value
...
def money_efficient():
	...
	if (Amount_money > Trade_mount):
		...
		for i in range(1, GRID_COUNT+1):
			...
			ws.cell(row=i, column=3).value = ws.cell(row=i, column=3).value - 100
			ws.cell(row=i, column=4).value = ws.cell(row=i, column=3).value * ws.cell(row=i, column=1).value
...
def money_balance_among_grid():
	while True:
		...
		if (money_high-money_low > ((float)(base_price) * 200)):
			ws.cell(row=low_index, column=3).value = ws.cell(row=low_index, column=3).value + 100
			ws.cell(row=low_index, column=4).value = ws.cell(row=low_index, column=3).value * ws.cell(row=low_index, column=1).value
			ws.cell(row=high_index, column=3).value = ws.cell(row=high_index, column=3).value - 100
			ws.cell(row=high_index, column=4).value = ws.cell(row=high_index, column=3).value * ws.cell(row=high_index, column=1).value
		else:
			break
